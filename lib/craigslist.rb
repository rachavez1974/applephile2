...
#     #it returns an array of cities belonging to a particular state
#     #1. find the state first.
#     #2. then traverse the hash to collect all the cities
#     state_cities_array = ""
#     states_cities_links.each do |states|
#       if states.has_key?(state_name)
#         state_cities_array = states[state_name].collect { |city| city.keys }
#       end
#     end
#     # clean_cities(state_cities_array.flatten!)
#     state_cities_array.flatten!  
#   end
...
#     #2. find the city
#     #3. get city's link
#     state_hash = states_cities_links.find { |state_hash| state_hash.has_key?(state) }
#     city_array = state_hash[state] 
#     city_hash = city_array.find { |cities_hash| cities_hash.has_key?(city) }
#     city_hash[city]
#   end
...
#     states_cities_links_array = []
#     #proceed by states, make hash, then push into array
#     #cities index implicitly starts from the first state
#     get_states_names.each_with_index do |state, index|
#       states_cities_links_array.push({ state => cities_and_links[index] })
#     end
#     states_cities_links_array
#   end
# end
