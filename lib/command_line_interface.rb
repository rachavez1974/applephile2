...
class CLI
  ...
  def grab_phones
    ...
    display_phone_info(@city.get_phones_by_price(gets.chomp))
  end
  ...
  def display_phone_info(phones)
    ...
    while choice != "exit"
      phones.each_with_index do |phone, index|
        puts "#{index + 1}: #{phone.description}"
        puts "Price: $#{phone.price}"
        puts "Phone's url: #{phone.url}"
        puts "Phone's City: #{phone.city.name}"
        puts separator
        ...
        if (index + 1) % 5 == 0
          ...
          if choice == "next"
            ...
            phone_url = phones[choice.to_i - 1].url 
...
#     puts "Price enter a price higher than 150 to see the list of phones, defualt price is 150.".colorize(:blue)
#     price = gets.chomp
#     phones = @city.get_phones_by_price(price)
#     display_phone_info(phones)
#   end
